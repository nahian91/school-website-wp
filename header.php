    ...
    <div class="dnt-container dnt-brand-wrapper">
        ...
        <div class="dnt-brand-right">
            <div class="dnt-brand-auth-buttons" style="display: flex; gap: 10px; align-items: center;">
                ...
                // 2. Try fetching student data via WordPress Logged-in User
                if ( is_user_logged_in() ) {
                    ...
                    if ( in_array( 'student', $user_roles, true ) ) {
                        ...
                        // Query DB by wp_user_id
                        $student_db = $wpdb->get_row( $wpdb->prepare( 
                            "SELECT full_name, student_id FROM {$table_students} WHERE wp_user_id = %d AND status = 'Active'", 
                            $current_user->ID 
                        ) );
                ...
                // 3. Fallback: Query via Custom Session ID if WP User check fails
                if ( ! $is_logged_in && ! empty( $_SESSION['dnt_student_id'] ) ) {
                ...
                ?>
                    ...
                    <div class="dnt-logged-in-badge" style="display: inline-flex; align-items: center; gap: 10px; background-color: #f8fafc; border: 1px solid #e2e8f0; padding: 6px 16px; border-radius: 30px; box-shadow: 0 1px 2px rgba(0,0,0,0.05);">
                        ...
                        <!-- Profile Link wrapper for avatar and full name -->
                        ...
                        <!-- Logout Link with Security Nonce -->
                        <a href="<?php echo esc_url( wp_logout_url( home_url( '/login?logout=success' ) ) ); ?>" style="color: #f42a41; text-decoration: none; font-size: 0.85rem; font-weight: 700; display: inline-flex; align-items: center; gap: 4px; transition: 0.2s;" onmouseover="this.style.opacity='0.8';" onmouseout="this.style.opacity='1';">
